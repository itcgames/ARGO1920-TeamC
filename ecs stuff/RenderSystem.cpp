...
void RenderSystem::render(SDL_Renderer* t_renderer)
{
	for (int i = 0; i < m_entities.size(); i++)
	{
		...
		{
			auto posComp = dynamic_cast<PositionComponent*>(m_entities.at(i)->getComponent(Component::ComponentType::Position));

			Uint8 prevRGBA[4];
			SDL_GetRenderDrawColor(t_renderer, &prevRGBA[0], &prevRGBA[1], &prevRGBA[2], &prevRGBA[3]);

			SDL_Rect rect;
			rect.x = posComp->getPos().x - 25;
			rect.y = posComp->getPos().y - 25;
			rect.w = 50;
			rect.h = 50;
